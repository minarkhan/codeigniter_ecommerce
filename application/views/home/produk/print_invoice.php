...
<body>
  ...
  <td>
    <div class="text-center align-middle mt-3">
      Total Bayar
      <h3>BDT. <?= rupiah($total['total'] + $total['ongkir']); ?><br>
  ...
  <table style="width: 100%; margin-top:20px">
    ...
    <tbody>
      ...
      foreach ($record->result_array() as $row) {
        ...
        if ($row['diskon'] != '0') {
          $diskon = "<del style='color:red'>BDT." . rupiah($row['harga_jual']) . "</del> &nbsp; ";
        } else {
          $diskon = "";
        }
        if (trim($row['gambar']) == '') {
      ...
      ?>
        <tr>
          ...
          <td> <?= $diskon ?> BDT. <?= rupiah($row['harga_jual'] - $row['diskon']) ?>
      ...
      <tr>
        ...
        <td class="border border-dark"><b>BDT.
            <span class="float-right">
              <?= rupiah($diskon_total); ?>
            </span>
          </b>
