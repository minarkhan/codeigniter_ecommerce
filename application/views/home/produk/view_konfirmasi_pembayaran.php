    ...
    <form action="<?= base_url('konfirmasi/form') ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
      ...
      if ($rows['kode_transaksi'] != '') { ?>
        ...
        <div class="form-group row">
          ...
          <div class="col-md-9">
            <input type='text' name='b' class='form-control' value="BDT. <?= rupiah($total['total'] + $total['ongkir'] + $total['kode_unik']); ?>" required>
          </div>
        </div>
      ...
      <div class="form-group row">
        ...
        <div class="col-md-9">
          <button type='submit' name='submit' class='btn btn-info btn-sm float-right'>Kirimkan</button>
        </div>
      </div>
